...
echo "2. 后端API: http://localhost:8080/api/v1/stats"
echo "3. 访问指南: http://localhost:8000/network-access-guide.html"
echo "4. 服务状态: http://localhost:8000/service-status.html"
echo ""
...
echo "   curl -v http://localhost:8080/api/v1/stats"
echo ""
echo "3. 重启服务:"
...
cat << 'EOF'
# 从外部测试（在另一台机器上运行）:
curl -v --connect-timeout 10 http://localhost:3000
curl -v --connect-timeout 10 http://localhost:8080/api/v1/stats
